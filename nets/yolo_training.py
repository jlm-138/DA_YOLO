...
class YOLOLoss(nn.Module):
    ...
    def __call__(self, predictions, targets, imgs,da,da_one,da_w,flag,epoch,flag_val=1):
        #-------------------------------------------#
        #   对输入进来的预测结果进行reshape
        ...
        da_loss = torch.zeros(1, device = device)   # 修改
        for i in range(len(da)):
            b,c,w,h=da[i].shape

            if flag==1:
                da_label= torch.ones_like(da[i])
            else:
                da_label=torch.zeros_like(da[i])

            da[i] = da[i].to(device)
            da_label=da_label.to(device)
            da_w[i]= da_w[i].to(device)

            loss_map = (da[i] - da_label).pow(2)  # 计算每个像素位置上的平方损失
            weighted_loss_map = loss_map * da_w[i]  # 使用域注意力权重加权
            # print('shape',da_w[i].shape)
            # print('value',da_w[i])

            # 计算总损失，这里使用均值作为总损失
            squared_loss = weighted_loss_map.mean()
            # squared_loss = F.mse_loss(torch.sigmoid(da[i]), da_label)
            # print('sq',squared_loss)

            da_loss+=self.da_weight1*squared_loss

        da_loss_one = torch.zeros(1, device=device)  # 修改
        for i in range(len(da_one)):
            b,c,w,h=da_one[i].shape
            da_one[i]=da_one[i].to(device)

            w = torch.ones_like(da_one[i])  # 初始化权重为1
            w=w.to(device)
            t=0.3
            a=10.0
            if epoch>200:  # 先训练一段时间在引入样本权重加权
                if flag==1:
                    mask = (da_one[i] > t) & (da_one[i] < 0.5)
                    w[mask] = 2 * (1 - da_one[i][mask])  # 增大权重

                    mask = (da_one[i] > (1-t))  # 减小权重
                    w[mask]=1.0/torch.exp((a*(da_one[i][mask]-(1-t))))

                    mask = (da_one[i] <t)  # 减小权重
                    w[mask] = (2*(1-t)) /torch.exp(a * (t-da_one[i][mask]))
                else:
                    mask = (da_one[i] > 0.5) & (da_one[i] < 1-t)
                    w[mask] = 2.0 * da_one[i][mask]  # 增大权重

                    mask = (da_one[i] > (1 - t))  # 减小权重
                    w[mask] = (2*t) / torch.exp(a * (da_one[i][mask] - (1 - t)))

                    mask = (da_one[i] < t)  # 减小权重
                    w[mask] = 1.0/ torch.exp(a * (t - da_one[i][mask]))

            myda_loss = F.binary_cross_entropy(da_one[i],torch.FloatTensor(da_one[i].data.size()).fill_(flag).to(device),reduction='none')
            myda_loss=w*myda_loss
            myda_loss=myda_loss.mean()
            # 计算损失
            # print('myda_loss',myda_loss)
            da_loss_one += myda_loss

        da_loss+=self.da_weight2*da_loss_one
        ...
        # print('class',cls_loss)
        # print('obj',obj_loss)
        bs          = tobj.shape[0]

        if flag==0 or flag_val==1:   #  验证的时候要有检测损失
            # loss    = box_loss + obj_loss + cls_loss
            loss = box_loss + obj_loss + cls_loss + da_loss
        else:
            loss =da_loss
