...
if __name__ == "__main__":
    ...
    image_ids = open(os.path.join(VOCdevkit_path, "VOC2007/ImageSets/Main/test.txt")).read().strip().split()  # 修改
        ...
        for image_id in tqdm(image_ids):
            with open(os.path.join(map_out_path, "ground-truth/"+image_id+".txt"), "w") as new_f:
                ...
                for obj in root.findall('object'):
                    ...
                    # myx = [int(float(bndbox.find(i).text)) for i in ['x0', 'x1', 'x2', 'x3']]
                    # myy = [int(float(bndbox.find(i).text)) for i in ['y0', 'y1', 'y2', 'y3']]
                    # xmin = min(myx)
                    # ymin = min(myy)
                    # xmax = max(myx)
                    # ymax = max(myy)
