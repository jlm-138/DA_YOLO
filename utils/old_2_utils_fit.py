...
def fit_one_epoch(model_train, model, ema, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val,epoch_step_ir, epoch_step_val_ir, gen, gen_val,gen_ir,gen_val_ir, Epoch, cuda, fp16, scaler, save_period, save_dir,save_dir_ir,train_dataset_ir,batch_size, local_rank=0):
    ...
    for iteration, batch in enumerate(gen):
        ...
        try:
            ...
            # 重新加载数据，重新创建数据加载器
            """gen_ir = DataLoader(train_dataset_ir, shuffle=True, batch_size=batch_size, num_workers=4,
                                pin_memory=True,
        ...
        if not fp16:
            ...
            loss_value_ir,da_lo_ir      = yolo_loss(outputs, targets_ir, images_ir,da,da_one,da_w,flag=1,epoch=epoch,flag_val=0)  # 不要检测损失

            #----------------------#
            #   反向传播
            #----------------------#
            loss_value_ir.backward()
            optimizer.step()
        if ema:
            ema.update(model_train)

        loss_ir += loss_value_ir.item()
        da_loss_ir+=da_lo_ir.item()
        # da_loss_ir=0
        # print('da_ir',da_lo_ir.item())
    ...
    if local_rank == 0:
        ...
        loss_history.append_loss(epoch + 1, loss_ir / epoch_step, val_loss_ir / epoch_step_val_ir)
        eval_callback.on_epoch_end(epoch + 1, model_train_eval)
        print('Epoch:'+ str(epoch + 1) + '/' + str(Epoch))
        print('Total Loss: %.3f || Val Loss: %.3f  Total Loss_ir: %.3f || Val Loss_ir: %.3f' % (loss / epoch_step, val_loss / epoch_step_val,loss_ir / epoch_step, val_loss_ir / epoch_step_val_ir))
        print('DA Loss: %.5f || DA_Val Loss: %.5f  DA Loss_ir: %.5f || DA_Val Loss_ir: %.5f' % (
        da_loss / epoch_step, da_val_loss / epoch_step_val, da_loss_ir / epoch_step, da_val_loss_ir / epoch_step_val_ir))
        #-----------------------------------------------#
        #   保存权值
        #-----------------------------------------------#
        if ema:
        ...
        if (epoch + 1) % save_period == 0 or epoch + 1 == Epoch:
            torch.save(save_state_dict, os.path.join(save_dir_ir, "ep%03d-loss%.3f-val_loss%.3f.pth" % (epoch + 1, loss_ir / epoch_step, val_loss_ir / epoch_step_val_ir)))
