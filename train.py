...
if __name__ == "__main__":
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path      = 'logs_ir/loss_2024_04_15_22_01_56/ep400-loss0.450-val_loss0.251.pth'
    #------------------------------------------------------#
    ...
    train_annotation_path_ir = '2007_train_ir.txt'
    val_annotation_path_ir = '2007_val_ir.txt'
    ...
    if model_path != '':
        ...
        if local_rank == 0:
            ...
            print("\nFail To Load Key:", str(no_load_key)[:500], "……\nFail To Load Key num:", len(no_load_key))   # 没有加载的结构
    ...
    if local_rank == 0:
        ...
        log_dir_ir = os.path.join(save_dir_ir, "loss_" + str(time_str))  # 修改
        loss_history_ir = LossHistory(log_dir_ir, model, input_shape=input_shape)
    else:
        loss_history    = None
    ...
    with open(train_annotation_path_ir, encoding='utf-8') as f:
        train_lines_ir = f.readlines()
    with open(val_annotation_path_ir, encoding='utf-8') as f:
        val_lines_ir = f.readlines()

    num_train_ir = len(train_lines_ir)
    num_val_ir = len(val_lines_ir)
    ...
    if True:
        ...
        epoch_step_ir = num_train_ir // batch_size
        epoch_step_val_ir = num_val_ir // batch_size
        ...
        train_dataset_ir = YoloDataset(train_lines_ir, input_shape, num_classes, anchors, anchors_mask,epoch_length=UnFreeze_Epoch,
                                       mosaic=mosaic, mixup=mixup, mosaic_prob=mosaic_prob, mixup_prob=mixup_prob,
                                    train=True, special_aug_ratio=special_aug_ratio)
        val_dataset_ir = YoloDataset(val_lines_ir, input_shape, num_classes, anchors, anchors_mask,
        ...
        gen_ir = DataLoader(train_dataset_ir, shuffle=shuffle, batch_size=batch_size, num_workers=num_workers,
                         pin_memory=True,
                         drop_last=True, collate_fn=yolo_dataset_collate, sampler=train_sampler,
                         worker_init_fn=partial(worker_init_fn, rank=rank, seed=seed))
        gen_val_ir = DataLoader(val_dataset_ir, shuffle=shuffle, batch_size=batch_size, num_workers=num_workers,
                             pin_memory=True,
                             drop_last=True, collate_fn=yolo_dataset_collate, sampler=val_sampler,
                             worker_init_fn=partial(worker_init_fn, rank=rank, seed=seed))
        ...
        if local_rank == 0:
            ...
            eval_callback_ir = EvalCallback(model, input_shape, anchors, anchors_mask, class_names, num_classes, val_lines_ir,log_dir_ir, Cuda, \
                                         eval_flag=eval_flag, period=eval_period)  # 修改
        else:
            eval_callback   = None
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            #   则解冻，并设置参数
            #---------------------------------------#
            if epoch >= Freeze_Epoch and not UnFreeze_flag and Freeze_Train:  # 解冻训练
                ...
                if ema:
                    ema.updates     = epoch_step * epoch   # 修改
                ...
                gen_ir = DataLoader(train_dataset_ir, shuffle=shuffle, batch_size=batch_size, num_workers=num_workers,pin_memory=True,
                                 drop_last=True, collate_fn=yolo_dataset_collate, sampler=train_sampler,
                                 worker_init_fn=partial(worker_init_fn, rank=rank, seed=seed))
                gen_val_ir = DataLoader(val_dataset_ir, shuffle=shuffle, batch_size=batch_size, num_workers=num_workers,pin_memory=True,
                                     drop_last=True, collate_fn=yolo_dataset_collate, sampler=val_sampler,
                                     worker_init_fn=partial(worker_init_fn, rank=rank, seed=seed))
            ...
            gen_ir.dataset.epoch_now = epoch
            gen_val_ir.dataset.epoch_now = epoch
            ...
            fit_one_epoch(model_train, model, ema, yolo_loss, loss_history_ir, eval_callback_ir, optimizer, epoch, epoch_step, epoch_step_val,epoch_step_ir, epoch_step_val_ir, gen, gen_val, gen_ir,gen_val_ir,UnFreeze_Epoch, Cuda, fp16, scaler, save_period, save_dir,save_dir_ir,train_dataset_ir,batch_size ,local_rank)
            
            if distributed:
                dist.barrier()

        if local_rank == 0:
            loss_history_ir.writer.close()
